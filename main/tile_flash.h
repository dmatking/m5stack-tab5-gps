...
// Read the tile at (tile_x, tile_y) into dst (MAP_TILE_SIZE x MAP_TILE_SIZE,
// contiguous, RGB565) if it falls within the embedded grid. Returns false
// (dst left untouched) for any tile outside the embedded area, or if the
// partition isn't present -- callers should fall back to synth_tile() in
// that case.

...
#
# Usage:
#   python tools/fetch_tiles.py 32.8896614814945 -97.34129452988256
...
def main():
    ...
    print(f"zoom={args.zoom} center_tile=({cx},{cy}) base=({base_tx},{base_ty}) grid={args.cols}x{args.rows}")
    ...
    for row in range(args.rows):
        for col in range(args.cols):
            tx = base_tx + col
            ty = base_ty + row
            url = f"https://tile.openstreetmap.org/{args.zoom}/{tx}/{ty}.png"
